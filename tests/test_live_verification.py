...
class TestLiveChainConfigs:
    ...
    async def test_arc_testnet_rpc_responds(self):
        ...
        assert chain_id == config.chain_id, (
            f"Chain ID mismatch: got {chain_id}, expected {config.chain_id}"
        )
        ...
        if result and len(result) > 2:
            ...
            try:
                ...
                if len(hex_data) >= 128:
                    ...
                    assert "USD" in name.upper() or "USDC" in name.upper(), (
                        f"Unexpected token name: {name}"
                    )
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                response = await client.post(
                    ...
                    json={"token": "USDC", "sources": [{"depositor": test_address}]},
                )
                print(
                    f"Balances endpoint: status={response.status_code}, body={response.text[:300]}"
                )
...
class TestEIP712Signatures:
    ...
    def test_payment_signature_is_recoverable(self):
        ...
        assert hasattr(payload, "signature"), (
            f"PaymentPayload missing signature attr: {type(payload)}"
        )
        assert hasattr(payload, "authorization"), "PaymentPayload missing authorization attr"
