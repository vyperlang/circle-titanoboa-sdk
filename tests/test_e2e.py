...
HAS_CIRCLE_CREDS = bool(os.environ.get("CIRCLE_API_KEY") and os.environ.get("CIRCLE_ENTITY_SECRET"))
...
class TestDepositFlow:
    ...
    def client(self):
        ...
        return GatewayClient(chain="arcTestnet", private_key=os.environ["PRIVATE_KEY"])
...
class TestPaymentSignatureFlow:
    ...
    def test_full_payment_flow(self):
        ...
        GatewayClient(chain="arcTestnet", private_key=private_key)  # validates key + chain
        ...
        print("  Received 402 with payment request for 0.05 USDC")
...
class TestCrossChainCompatibility:
    ...
    async def test_arc_and_base_sepolia_parallel(self):
        ...
        print(
            f"Querying Arc Testnet (chain {arc_client.chain_id}) and Base Sepolia (chain {base_client.chain_id})..."
        )
...
class TestProgrammableWalletsWithGateway:
    ...
    @pytest.mark.skipif(not HAS_CIRCLE_CREDS, reason=SKIP_REASON_CIRCLE)
